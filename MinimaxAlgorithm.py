...
Evaluation function: takes in a state of the game (board arrangement) and boils it down to a real number evaluation of the state. Ex: function gives higher scores to board states in which the player of interest has more of their pieces on the board then the opponent we want the function to assign an infinity score to the board arrangement in which the opponent's king is checkmate; meaning that our player of interest is guaranteed to win the game we could take the opponent's actions into consideration once we've moved.
